...
class DataSet:
    def __init__(self, x: np.ndarray, y: np.ndarray, name=None, shuffle=True):
        ...
        self.dim_x = tuple(x.shape[1:])
        self.dim_y = tuple(y.shape[1:])
    ...
    def random_split(self, ratio: float) -> ('DataSet', 'DataSet'):
